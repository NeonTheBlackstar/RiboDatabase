...
unless ($@) {
  Log::Log4perl::init('/root/meme/etc/logging.conf');
  $logger = Log::Log4perl->get_logger('meme.service.utils');
}

my $service_invocation_log_dir = '/root/meme/LOGS';
my $tmpdir = '';
...
sub update_status {
  ...
  my $template = HTML::Template->new(filename => '/root/meme/etc/job_status.tmpl');
